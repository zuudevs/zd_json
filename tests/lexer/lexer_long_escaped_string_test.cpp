...
TEST(JsonLexerTest, LongAndEscapedStrings) {
    ...
    const auto tokensLong = zuu::json::lexer::Tokenize(jsonLongPayload);
    const auto valuesLong = zuu::json::lexer::LexValues(jsonLongPayload, tokensLong);

    ASSERT_GE(valuesLong.size(), 5u);

    // "hello\nworld" has escape -> String
    EXPECT_EQ(valuesLong[2].type, zuu::json::enums::JsonType::String);
    EXPECT_EQ(valuesLong[2].value, R"("hello\nworld")");

    // long string > 16 chars -> String
    EXPECT_EQ(valuesLong[4].type, zuu::json::enums::JsonType::String);
