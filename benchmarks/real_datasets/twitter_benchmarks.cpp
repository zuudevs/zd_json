...
void
    Parse_Twitter(benchmark::State& state) {
...
void
    SerializeCompact_Twitter(benchmark::State& state) {
    const auto& json = Datasets::GetTwitter();
    auto docResult = zuu::json::Parse(json);
...
void
    SerializePretty_Twitter(benchmark::State& state) {
    const auto& json = Datasets::GetTwitter();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
...
void
    Tokenize_Twitter(benchmark::State& state) {
    const auto& json = Datasets::GetTwitter();
    const auto jsonSize = static_cast<int64_t>(json.size());
...
void
    Traverse_Twitter(benchmark::State& state) {
    const auto& json = Datasets::GetTwitter();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
