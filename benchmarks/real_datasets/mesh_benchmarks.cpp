/**
 * @file mesh_benchmarks.cpp
 * @author zuudevs (zuudevs@gmail.com)
 * @brief Benchmark implementations for mesh.json dataset.
 * @version 0.8.0
 * @date 2026-07-29
 *
...
void
    Parse_Mesh(benchmark::State& state) {
    const auto& json = Datasets::GetMesh();
...
void
    SerializeCompact_Mesh(benchmark::State& state) {
    const auto& json = Datasets::GetMesh();
    auto docResult = zuu::json::Parse(json);
...
void
    SerializePretty_Mesh(benchmark::State& state) {
    const auto& json = Datasets::GetMesh();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
...
void
    Tokenize_Mesh(benchmark::State& state) {
    const auto& json = Datasets::GetMesh();
    const auto jsonSize = static_cast<int64_t>(json.size());
...
void
    Traverse_Mesh(benchmark::State& state) {
    const auto& json = Datasets::GetMesh();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
