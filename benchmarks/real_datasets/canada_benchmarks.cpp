/**
 * @file canada_benchmarks.cpp
 * @author zuudevs (zuudevs@gmail.com)
 * @brief Benchmark implementations for canada.json dataset.
 * @version 0.8.0
 * @date 2026-07-29
 *
...
void
    Parse_Canada(benchmark::State& state) {
    const auto& json = Datasets::GetCanada();
...
void
    SerializeCompact_Canada(benchmark::State& state) {
    const auto& json = Datasets::GetCanada();
    auto docResult = zuu::json::Parse(json);
...
void
    SerializePretty_Canada(benchmark::State& state) {
    const auto& json = Datasets::GetCanada();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
...
void
    Tokenize_Canada(benchmark::State& state) {
    const auto& json = Datasets::GetCanada();
    const auto jsonSize = static_cast<int64_t>(json.size());
...
void
    Traverse_Canada(benchmark::State& state) {
    const auto& json = Datasets::GetCanada();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
