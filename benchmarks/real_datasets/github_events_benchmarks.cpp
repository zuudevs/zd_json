/**
 * @file github_events_benchmarks.cpp
 * @author zuudevs (zuudevs@gmail.com)
 * @brief Benchmark implementations for github_events.json dataset.
 * @version 0.8.0
 * @date 2026-07-29
 *
...
void
    Parse_GithubEvents(benchmark::State& state) {
    const auto& json = Datasets::GetGithubEvents();
...
void
    SerializeCompact_GithubEvents(benchmark::State& state) {
    const auto& json = Datasets::GetGithubEvents();
    auto docResult = zuu::json::Parse(json);
...
void
    SerializePretty_GithubEvents(benchmark::State& state) {
    const auto& json = Datasets::GetGithubEvents();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
...
void
    Tokenize_GithubEvents(benchmark::State& state) {
    const auto& json = Datasets::GetGithubEvents();
    const auto jsonSize = static_cast<int64_t>(json.size());
...
void
    Traverse_GithubEvents(benchmark::State& state) {
    const auto& json = Datasets::GetGithubEvents();
    auto docResult = zuu::json::Parse(json);
    if (!docResult.has_value()) {
        state.SkipWithError("Parse failed");
